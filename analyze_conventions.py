...
def detect_column_limit(files, percentile=0.90, tab_width=4):
    ...
    # Snap to a standard value if close
    for standard in STANDARD_LIMITS:
        if abs(detected - standard) <= SNAP_THRESHOLD:
            return standard
...
def detect_qualifier_alignment(files):
...
def detect_access_modifier_offset(files):
...
def detect_max_empty_lines(files):
...
def detect_numeric_literal_case(files):
    ...
    # Match float literals with exponent: e.g. 1.5e10, 3.14E-2
    exponent_pattern = re.compile(r'[0-9](?:[.eE])([eE])([+-]?[0-9]+)')
    # Match integer suffixes: e.g. 10u, 10l, 10ull, 10UL, 10ULL
    suffix_pattern = re.compile(r'\b[0-9][0-9a-fA-FxX]*(u|l|ul|lu|ull|llu|ll)(\b)')
...
def main():
    ...
    parser.add_argument("path", help="Path to the source directory to analyze.")
    parser.add_argument(
        "--percentile",
        type=float,
        default=0.95,
    ...
    if not files:
        print(f"No C/C++ source files found in '{args.path}'.", file=sys.stderr)
        sys.exit(1)

    print(f"Analyzing {len(files)} files in '{args.path}'...", file=sys.stderr)

    indent_width = detect_indent_width(files)
    column_limit = detect_column_limit(files, args.percentile, indent_width or 4)
    ...
    hex_digit_case, prefix_case, exponent_case, suffix_case = detect_numeric_literal_case(files)

    print(f"# Detected conventions from {len(files)} files", file=sys.stderr)

    # Output valid YAML
    print(f"DisableFormat: false")
    ...
    print(f"DerivePointerAlignment: true")
    print(f"NumericLiteralCase:")
