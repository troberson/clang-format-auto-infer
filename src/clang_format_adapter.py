...
def build_search_space(
    ...
    for full_path, option_info in base_options.items():
        ...
        if analysis_results and full_path in analysis_results:
            ...
            # All analyzer-detected options are forced. The analyzer finds these
            # deterministically (ColumnLimit, IndentWidth, Language, etc.) and
            # they should not be re-optimized by global-polish. Only guessed
            # options remain mutable for empirical validation.
            if is_guessed:
                confidence = "guessed"
            else:
                confidence = "forced"
